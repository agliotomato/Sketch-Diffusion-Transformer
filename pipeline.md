...
학습 완료 후 `inference_sd3_5.py` 실행 시 동작 흐름은 다음과 같습니다.

1. **LoRA 로드**
   - 학습된 LoRA 가중치를 로드하여 머리카락 질감 및 디테일 표현을 적용합니다.

2. **텍스트 인코더 활성화**
   - 학습 단계에서는 비활성화되어 있으나, 추론 단계에서는 활성화합니다.
   - 텍스트 프롬프트를 통해 머리 색상, 스타일 등의 속성을 제어합니다.
     - 예: `"red hair"`, `"blonde hair"`

3. **최종 결과 합성**
   - **형태(Structure)**: ControlNet이 스케치를 기반으로 기하학적 구조를 제어합니다.
   - **질감(Texture)**: LoRA가 실사 기반 머리카락 질감 및 세부 표현을 담당합니다.
   - **색상(Color)**: 텍스트 인코더 및 프롬프트 조건에 의해 결정됩니다.
   - **주의 사항**:
     - 스케치 이미지의 색상 정보는 사용되지 않음
     - ControlNet은 스케치를 흑백 구조 정보(에지/선 구조)로만 인식
     - 머리 색상은 반드시 텍스트 프롬프트를 통해 지정해야 함


## 5. 왜 Reference Attention인가?
단순 인페인팅(Inpainting) 대신 **Reference Attention** 기법을 사용
